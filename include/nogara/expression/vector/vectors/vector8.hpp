#ifndef NOGARA_EXPRESSION_VECTOR8_HPP
#define NOGARA_EXPRESSION_VECTOR8_HPP

//////////////////////////////////////////////////////
//! @file nogara/expression/vector/vectors/vector8.hpp
//! @brief 8次元ベクトルクラス
//////////////////////////////////////////////////////
...
template<typename T>
class vector8 : public vector_container<vector8<T>, T>{
    // 自身の型
    typedef vector8<T>                     self_type;
    // 親の型
    typedef vector_container<self_type, T> base_type;
    // コンテナ型
    typedef expression_container<T, 8>     container_type;
    // コンテナの参照型
...
public:
    ...
    //! @brief コンストラクタ
    vector8():container(){}
    //! @brief コピーコンストラクタ 他の8次元ベクトルからの初期化
    vector8(vector8 const& other):container(other.data()){}
    //! @brief コンストラクタ 要素の初期化
    vector8(reference _0, reference _1, reference _2, reference _3,
            reference _4, reference _5, reference _6, reference _7):
        container(_0, _1, _2, _3, _4, _5, _6, _7){}
    //! @brief コンストラクタ 配列からの初期化
    vector8(value_type (&ary)[Dimension::value]):
    container(ary[0], ary[1], ary[2], ary[3],
              ary[4], ary[5], ary[6], ary[7]){}

    //! @brief コンストラクタ ベクトル式からの初期化
    template<class E>
    vector8(vector_expression<E> const& e):
        container(get<0>(e), get<1>(e), get<2>(e), get<3>(e),
                  get<4>(e), get<5>(e), get<6>(e), get<7>(e)){}

#if defined(NG_CPP0X)
    //! @brief ムーブコンストラクタ。C++0X対応のコンパイラであれば定義される
    vector8(vector8&& other):container(std::forward<container_type>(other.container)){}
#endif

    //! @brief 他の3次元ベクトルから代入
    self_type& operator =(vector8 const& v){
        if(&v != this){
            container = v.data();
        }
        return *this;
...
template<typename T>
vector8<T> const vector8<T>::zero(T(), T(), T(), T(),
                                  T(), T(), T(), T());
...
//!        8次元ベクトルに対する特殊化
///////////////////////////////////////////
template<class T> struct vector_type<T, 8>{
    typedef vector8<T> type;
};

__NOGARA_EXPRESSION_END

__NOGARA_BEGIN
using expression::vector8;
__NOGARA_END
