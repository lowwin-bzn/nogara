#ifndef NOGARA_EXPRESSION_VECTOR10_HPP
#define NOGARA_EXPRESSION_VECTOR10_HPP

///////////////////////////////////////////////////////
//! @file nogara/expression/vector/vectors/vector10.hpp
//! @brief 10次元ベクトルクラス
//////////////////////////////////////////////////////
...
template<typename T>
class vector10 : public vector_container<vector10<T>, T>{
    // 自身の型
    typedef vector10<T>                     self_type;
    // 親の型
    typedef vector_container<self_type, T> base_type;
    // コンテナ型
    typedef expression_container<T, 10>     container_type;
    // コンテナの参照型
...
public:
    ...
    //! @brief コンストラクタ
    vector10():container(){}
    //! @brief コピーコンストラクタ 他の10次元ベクトルからの初期化
    vector10(vector10 const& other):container(other.data()){}
    //! @brief コンストラクタ 要素の初期化
    vector10(reference _0, reference _1, reference _2, reference _3,
             reference _4, reference _5, reference _6, reference _7,
             reference _8, reference _9):
        container(_0, _1, _2, _3,
                  _4, _5, _6, _7,
                  _8, _9){}
    //! @brief コンストラクタ 配列からの初期化
    vector10(value_type (&ary)[Dimension::value]):
    ...
    template<class E>
    vector10(vector_expression<E> const& e):
    ...
    vector10(vector10&& other):container(std::forward<container_type>(other.container)){}
#endif

    //! @brief 他の3次元ベクトルから代入
    self_type& operator =(vector10 const& v){
        if(&v != this){
            container = v.data();
        }
        return *this;
...
template<typename T>
vector10<T> const vector10<T>::zero(T(), T(), T(), T(),
                                    T(), T(), T(), T(),
                                    T(), T());
...
//!        10次元ベクトルに対する特殊化
///////////////////////////////////////////
template<class T> struct vector_type<T, 10>{
    typedef vector10<T> type;
};

__NOGARA_EXPRESSION_END

__NOGARA_BEGIN
using expression::vector10;
__NOGARA_END
