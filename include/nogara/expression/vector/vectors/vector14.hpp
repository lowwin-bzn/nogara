#ifndef NOGARA_EXPRESSION_VECTOR14_HPP
#define NOGARA_EXPRESSION_VECTOR14_HPP

///////////////////////////////////////////////////////
//! @file nogara/expression/vector/vectors/vector14.hpp
//! @brief 14次元ベクトルクラス
///////////////////////////////////////////////////////
...
template<typename T>
class vector14 : public vector_container<vector14<T>, T>{
    // 自身の型
    typedef vector14<T>                     self_type;
    // 親の型
    typedef vector_container<self_type, T> base_type;
    // コンテナ型
    typedef expression_container<T, 14>     container_type;
    // コンテナの参照型
...
public:
    ...
    //! @brief コンストラクタ
    vector14():container(){}
    //! @brief コピーコンストラクタ 他の14次元ベクトルからの初期化
    vector14(vector14 const& other):container(other.data()){}
    //! @brief コンストラクタ 要素の初期化
    vector14(reference _0,  reference _1, reference _2,  reference _3,
             reference _4,  reference _5, reference _6,  reference _7,
             reference _8,  reference _9, reference _10, reference _11,
             reference _12, reference _13):
        container(_0,  _1, _2,  _3,
                  _4,  _5, _6,  _7,
                  _8,  _9, _10, _11,
                  _12, _13){}
    //! @brief コンストラクタ 配列からの初期化
    vector14(value_type (&ary)[Dimension::value]):
    ...
    template<class E>
    vector14(vector_expression<E> const& e):
    ...
    vector14(vector14&& other):container(std::forward<container_type>(other.container)){}
#endif

    //! @brief 他の3次元ベクトルから代入
    self_type& operator =(vector14 const& v){
        if(&v != this){
            container = v.data();
        }
        return *this;
...
template<typename T>
vector14<T> const vector14<T>::zero(T(), T(), T(), T(),
                                    T(), T(), T(), T(),
                                    T(), T(), T(), T(),
                                    T(), T());
...
//!        14次元ベクトルに対する特殊化
///////////////////////////////////////////
template<class T> struct vector_type<T, 14>{
    typedef vector14<T> type;
};

__NOGARA_EXPRESSION_END

__NOGARA_BEGIN
using expression::vector14;
__NOGARA_END
