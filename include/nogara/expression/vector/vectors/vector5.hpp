#ifndef NOGARA_EXPRESSION_VECTOR5_HPP
#define NOGARA_EXPRESSION_VECTOR5_HPP

//////////////////////////////////////////////////////
//! @file nogara/expression/vector/vectors/vector5.hpp
//! @brief 5次元ベクトルクラス
//////////////////////////////////////////////////////
...
template<typename T>
class vector5 : public vector_container<vector5<T>, T>{
    // 自身の型
    typedef vector5<T>                     self_type;
    // 親の型
    typedef vector_container<self_type, T> base_type;
    // コンテナ型
    typedef expression_container<T, 5>     container_type;
    // コンテナの参照型
...
public:
    ...
    //! @brief コンストラクタ
    vector5():container(){}
    //! @brief コピーコンストラクタ 他の5次元ベクトルからの初期化
    vector5(vector5 const& other):container(other.data()){}
    //! @brief コンストラクタ 要素の初期化
    vector5(reference _0, reference _1, reference _2, reference _3,
            reference _4):
        container(_1, _2, _3, _4, _5){}
    //! @brief コンストラクタ 配列からの初期化
    vector5(value_type (&ary)[Dimension::value]):
    container(ary[0], ary[1], ary[2], ary[3], ary[4]){}

    //! @brief コンストラクタ ベクトル式からの初期化
    template<class E>
    vector5(vector_expression<E> const& e):
        container(get<0>(e), get<1>(e), get<2>(e), get<3>(e),
                  get<4>(e)){}

#if defined(NG_CPP0X)
    //! @brief ムーブコンストラクタ。C++0X対応のコンパイラであれば定義される
    vector5(vector5&& other):container(std::forward<container_type>(other.container)){}
#endif

    //! @brief 他の3次元ベクトルから代入
    self_type& operator =(vector5 const& v){
        if(&v != this){
            container = v.data();
        }
        return *this;
...
template<typename T>
vector5<T> const vector5<T>::zero(T(0), T(0), T(0), T(0), T(0));

///////////////////////////////////////////
//! @brief 次元数と等しいベクトル型を返す
//!        5次元ベクトルに対する特殊化
///////////////////////////////////////////
template<class T> struct vector_type<T, 5>{
    typedef vector5<T> type;
};

__NOGARA_EXPRESSION_END

__NOGARA_BEGIN
using expression::vector5;
__NOGARA_END
