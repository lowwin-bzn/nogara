#ifndef NOGARA_EXPRESSION_VECTOR6_HPP
#define NOGARA_EXPRESSION_VECTOR6_HPP

//////////////////////////////////////////////////////
//! @file nogara/expression/vector/vectors/vector6.hpp
//! @brief 6次元ベクトルクラス
//////////////////////////////////////////////////////
...
template<typename T>
class vector6 : public vector_container<vector6<T>, T>{
    // 自身の型
    typedef vector6<T>                     self_type;
    // 親の型
    typedef vector_container<self_type, T> base_type;
    // コンテナ型
    typedef expression_container<T, 6>     container_type;
    // コンテナの参照型
...
public:
    ...
    //! @brief コンストラクタ
    vector6():container(){}
    //! @brief コピーコンストラクタ 他の6次元ベクトルからの初期化
    vector6(vector6 const& other):container(other.data()){}
    //! @brief コンストラクタ 要素の初期化
    vector6(reference _0, reference _1, reference _2, reference _3,
            reference _4, reference _5):
        container(_0, _1, _2, _3, _4, _5){}
    //! @brief コンストラクタ 配列からの初期化
    vector6(value_type (&ary)[Dimension::value]):
    container(ary[0], ary[1], ary[2], ary[3],
              ary[4], ary[5]){}

    //! @brief コンストラクタ ベクトル式からの初期化
    template<class E>
    vector6(vector_expression<E> const& e):
        container(get<0>(e), get<1>(e), get<2>(e), get<3>(e),
                  get<4>(e), get<5>(e)){}

#if defined(NG_CPP0X)
    //! @brief ムーブコンストラクタ。C++0X対応のコンパイラであれば定義される
    vector6(vector6&& other):container(std::forward<container_type>(other.container)){}
#endif

    //! @brief 他の3次元ベクトルから代入
    self_type& operator =(vector6 const& v){
        if(&v != this){
            container = v.data();
        }
        return *this;
...
template<typename T>
vector6<T> const vector6<T>::zero(T(0), T(0), T(0), T(0),
                                  T(0), T(0));
...
//!        6次元ベクトルに対する特殊化
///////////////////////////////////////////
template<class T> struct vector_type<T, 6>{
