#ifndef NOGARA_EXPRESSION_VECTOR13_HPP
#define NOGARA_EXPRESSION_VECTOR13_HPP

///////////////////////////////////////////////////////
//! @file nogara/expression/vector/vectors/vector13.hpp
//! @brief 13次元ベクトルクラス
///////////////////////////////////////////////////////
...
template<typename T>
class vector13 : public vector_container<vector13<T>, T>{
    // 自身の型
    typedef vector13<T>                     self_type;
    // 親の型
    typedef vector_container<self_type, T> base_type;
    // コンテナ型
    typedef expression_container<T, 13>     container_type;
    // コンテナの参照型
...
public:
    ...
    //! @brief コンストラクタ
    vector13():container(){}
    //! @brief コピーコンストラクタ 他の13次元ベクトルからの初期化
    vector13(vector13 const& other):container(other.data()){}
    //! @brief コンストラクタ 要素の初期化
    vector13(reference _0, reference _1, reference _2,  reference _3,
    ...
    vector13(value_type (&ary)[Dimension::value]):
    ...
    template<class E>
    vector13(vector_expression<E> const& e):
    ...
    vector13(vector13&& other):container(std::forward<container_type>(other.container)){}
#endif

    //! @brief 他の3次元ベクトルから代入
    self_type& operator =(vector13 const& v){
        if(&v != this){
            container = v.data();
        }
        return *this;
...
template<typename T>
vector13<T> const vector13<T>::zero(T(), T(), T(), T(),
                                    T(), T(), T(), T(),
                                    T(), T(), T(), T(),
                                    T());
...
//!        13次元ベクトルに対する特殊化
///////////////////////////////////////////
template<class T> struct vector_type<T, 13>{
    typedef vector13<T> type;
};

__NOGARA_EXPRESSION_END

__NOGARA_BEGIN
using expression::vector13;
__NOGARA_END
