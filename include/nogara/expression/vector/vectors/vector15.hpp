#ifndef NOGARA_EXPRESSION_VECTOR15_HPP
#define NOGARA_EXPRESSION_VECTOR15_HPP

///////////////////////////////////////////////////////
//! @file nogara/expression/vector/vectors/vector15.hpp
//! @brief 15次元ベクトルクラス
///////////////////////////////////////////////////////
...
template<typename T>
class vector15 : public vector_container<vector15<T>, T>{
    // 自身の型
    typedef vector15<T>                     self_type;
    // 親の型
    typedef vector_container<self_type, T> base_type;
    // コンテナ型
    typedef expression_container<T, 15>     container_type;
    // コンテナの参照型
...
public:
    ...
    //! @brief コンストラクタ
    vector15():container(){}
    //! @brief コピーコンストラクタ 他の15次元ベクトルからの初期化
    vector15(vector15 const& other):container(other.data()){}
    //! @brief コンストラクタ 要素の初期化
    vector15(reference _0,  reference _1,  reference _2,  reference _3,
             reference _4,  reference _5,  reference _6,  reference _7,
             reference _8,  reference _9,  reference _10, reference _11,
             reference _12, reference _13, reference _14):
        container(_0,  _1,  _2,  _3,
                  _4,  _5,  _6,  _7,
                  _8,  _9,  _10, _11,
                  _12, _13, _14){}
    //! @brief コンストラクタ 配列からの初期化
    vector15(value_type (&ary)[Dimension::value]):
    container(ary[0],  ary[1],  ary[2],  ary[3],
              ary[4],  ary[5],  ary[6],  ary[7],
              ary[8],  ary[9],  ary[10], ary[11],
              ary[12], ary[13], ary[14]){}

    //! @brief コンストラクタ ベクトル式からの初期化
    template<class E>
    vector15(vector_expression<E> const& e):
        container(get<0>(e),  get<1>(e),  get<2>(e),  get<3>(e),
                  get<4>(e),  get<5>(e),  get<6>(e),  get<7>(e),
                  get<8>(e),  get<9>(e),  get<10>(e), get<11>(e),
                  get<12>(e), get<13>(e), get<14>(e)){}

#if defined(NG_CPP0X)
    //! @brief ムーブコンストラクタ。C++0X対応のコンパイラであれば定義される
    vector15(vector15&& other):container(std::forward<container_type>(other.container)){}
#endif

    //! @brief 他の3次元ベクトルから代入
    self_type& operator =(vector15 const& v){
        if(&v != this){
            container = v.data();
        }
        return *this;
...
template<typename T>
vector15<T> const vector15<T>::zero(T(), T(), T(), T(),
                                    T(), T(), T(), T(),
                                    T(), T(), T(), T(),
                                    T(), T(), T());
...
//!        15次元ベクトルに対する特殊化
///////////////////////////////////////////
template<class T> struct vector_type<T, 15>{
    typedef vector15<T> type;
};

__NOGARA_EXPRESSION_END

__NOGARA_BEGIN
using expression::vector15;
__NOGARA_END
