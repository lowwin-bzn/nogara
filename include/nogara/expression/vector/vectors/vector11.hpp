#ifndef NOGARA_EXPRESSION_VECTOR11_HPP
#define NOGARA_EXPRESSION_VECTOR11_HPP

///////////////////////////////////////////////////////
//! @file nogara/expression/vector/vectors/vector11.hpp
//! @brief 11次元ベクトルクラス
///////////////////////////////////////////////////////
...
template<typename T>
class vector11 : public vector_container<vector11<T>, T>{
    // 自身の型
    typedef vector11<T>                     self_type;
    // 親の型
    typedef vector_container<self_type, T> base_type;
    // コンテナ型
    typedef expression_container<T, 11>     container_type;
    // コンテナの参照型
...
public:
    ...
    //! @brief コンストラクタ
    vector11():container(){}
    //! @brief コピーコンストラクタ 他の11次元ベクトルからの初期化
    vector11(vector11 const& other):container(other.data()){}
    //! @brief コンストラクタ 要素の初期化
    vector11(reference _0, reference _1, reference _2, reference _3,
             reference _4, reference _5, reference _6, reference _7,
             reference _8, reference _9, reference _10):
        container(_0, _1, _2, _3,
                  _4, _5, _6, _7,
                  _8, _9, _10){}
    //! @brief コンストラクタ 配列からの初期化
    vector11(value_type (&ary)[Dimension::value]):
    ...
    template<class E>
    vector11(vector_expression<E> const& e):
        container(get<0>(e), get<1>(e), get<2>(e), get<3>(e),
                  get<4>(e), get<5>(e), get<6>(e), get<7>(e),
                  get<8>(e), get<9>(e), get<10>(e)){}

#if defined(NG_CPP0X)
    //! @brief ムーブコンストラクタ。C++0X対応のコンパイラであれば定義される
    vector11(vector11&& other):container(std::forward<container_type>(other.container)){}
#endif

    //! @brief 他の3次元ベクトルから代入
    self_type& operator =(vector11 const& v){
        if(&v != this){
            container = v.data();
        }
        return *this;
...
template<typename T>
vector11<T> const vector11<T>::zero(T(), T(), T(), T(),
                                    T(), T(), T(), T(),
                                    T(), T(), T());
...
//!        11次元ベクトルに対する特殊化
///////////////////////////////////////////
template<class T> struct vector_type<T, 11>{
    typedef vector11<T> type;
};

__NOGARA_EXPRESSION_END

__NOGARA_BEGIN
using expression::vector11;
__NOGARA_END
