#ifndef NOGARA_EXPRESSION_VECTOR12_HPP
#define NOGARA_EXPRESSION_VECTOR12_HPP

///////////////////////////////////////////////////////
//! @file nogara/expression/vector/vectors/vector12.hpp
//! @brief 12次元ベクトルクラス
///////////////////////////////////////////////////////
...
template<typename T>
class vector12 : public vector_container<vector12<T>, T>{
    // 自身の型
    typedef vector12<T>                     self_type;
    // 親の型
    typedef vector_container<self_type, T> base_type;
    // コンテナ型
    typedef expression_container<T, 12>     container_type;
    // コンテナの参照型
...
public:
    ...
    //! @brief コンストラクタ
    vector12():container(){}
    //! @brief コピーコンストラクタ 他の12次元ベクトルからの初期化
    vector12(vector12 const& other):container(other.data()){}
    //! @brief コンストラクタ 要素の初期化
    vector12(reference _0, reference _1, reference _2,  reference _3,
    ...
    vector12(value_type (&ary)[Dimension::value]):
    ...
    template<class E>
    vector12(vector_expression<E> const& e):
    ...
    vector12(vector12&& other):container(std::forward<container_type>(other.container)){}
#endif

    //! @brief 他の3次元ベクトルから代入
    self_type& operator =(vector12 const& v){
        if(&v != this){
            container = v.data();
        }
        return *this;
...
template<typename T>
vector12<T> const vector12<T>::zero(T(), T(), T(), T(),
                                    T(), T(), T(), T(),
                                    T(), T(), T(), T());
...
//!        12次元ベクトルに対する特殊化
///////////////////////////////////////////
template<class T> struct vector_type<T, 12>{
    typedef vector12<T> type;
};

__NOGARA_EXPRESSION_END

__NOGARA_BEGIN
using expression::vector12;
__NOGARA_END
