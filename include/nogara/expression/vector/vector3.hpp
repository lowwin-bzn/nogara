#ifndef NOGARA_EXPRESSION_VECTOR3_HPP
#define NOGARA_EXPRESSION_VECTOR3_HPP

//////////////////////////////////////////////
//! @file nogara/expression/vector/vector3.hpp
//! @brief 3次元ベクトルクラス
//////////////////////////////////////////////
...
//////////////////////////////
template<typename T>
class vector3 : public vector_container<vector3<T>, T>{
    // 自身の型
    typedef vector3<T>                     self_type;
    // 親の型
    typedef vector_container<self_type, T> base_type;
    // コンテナ型
    typedef expression_container<T, 3>     container_type;
    // コンテナの参照型
...
public:
    ...
    //! @brief コンストラクタ
    vector3():container(){}
    //! @brief コピーコンストラクタ 他の3次元ベクトルからの初期化
    vector3(vector3 const& other):container(other.data()){}
    //! @brief コンストラクタ 要素の初期化
    vector3(T const& x, T const& y, T const& z):container(x, y, z){}

    //! @brief コンストラクタ 配列からの初期化
    vector3(value_type const (&ary)[Dimension::value]):container(ary[0], ary[1], ary[2]){}

    //! @brief コンストラクタ ベクトル式からの初期化
    template<class E>
    vector3(vector_expression<E> const& e):container(get<0>(e), get<1>(e), get<2>(e)){}
    //! @brief コンストラクタ 2次元ベクトル式からの初期化
    //!        第3要素には1を代入
    template<class E>
    vector3(vector_expression<E> const& e,
            typename boost::enable_if<dimension_equal_to_c<E, 2> >::type* = 0):
        container(get<0>(e), get<1>(e), value_type(1)){}

#if defined(NG_CPP0X)
    //! @brief ムーブコンストラクタ。C++0X対応のコンパイラであれば定義される
    vector3(vector3&& other):container(std::forward<container_type>(other.container)){}
#endif

    //! @brief 他の3次元ベクトルから代入
    self_type& operator =(vector3 const& v){
        if(&v != this){
            container = v.data();
        }
        return *this;
    ...
    operator =(vector_expression<E> const& e){
        x(get<0>(e)); y(get<1>(e)); z(get<2>(e));
        return *this;
    }

#if defined(NG_CPP0X)
    //! @brief ムーブ代入 C++0X対応のコンパイラであれば定義される
    self_type& operator =(vector3&& v){
...
template<typename T>
vector3<T> const vector3<T>::zero(T(0), T(0), T(0));
template<typename T>
vector3<T> const vector3<T>::unit_x(T(1), T(0), T(0));
template<typename T>
vector3<T> const vector3<T>::unit_y(T(0), T(1), T(0));
template<typename T>
vector3<T> const vector3<T>::unit_z(T(0), T(0), T(1));

///////////////////////////////////////////
//! @brief 次元数と等しいベクトル型を返す
//!        3次元ベクトルに対する特殊化
///////////////////////////////////////////
template<class T> struct vector_type<T, 3>{
